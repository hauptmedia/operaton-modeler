  ...
  componentDidMount() {
    ...
    getGlobal('backend').on('client:connector-templates-update-success', (_, hasNew, warnings = []) => {
      ...
      if (warnings.length) {
        displayNotification({
          type: 'warning',
          title: 'Operaton Connector templates updated with warnings',
      ...
      displayNotification({
        type: 'success',
        title: hasNew ? 'Operaton Connector templates updated' : 'Operaton Connector templates up to date',
    ...
    getGlobal('backend').on('client:connector-templates-update-error', (_, message) => {
      displayNotification({
        type: 'error',
        title: 'Error updating Operaton Connector templates',
        content: <Fragment>
          <span>{ message }</span>
          <br />
          <a href={ DOCUMENTATION_URL }>Learn more</a>
